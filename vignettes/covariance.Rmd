...
every within-subject cell and a correlation of 0.5 between every pair by
default. All four accept `within_covariance()` when repeated measurements have
different standard deviations or correlations.
...
For `power_unbalanced()`, standard deviations belong in `cell_design()`, so
its covariance argument accepts `unbalanced_covariance()` correlations only.
